...
@section('content')
	<div class="flex flex-col gap-12 items-center w-full h-full overflow-x-hidden">
		<!-- Movie showcase -->
		<div class="relative w-full h-72 md:h-[38rem] overflow-hidden md:min-w-[70rem]" x-data="carousel()"
		     x-init="startCarousel; setMovies({{$movies}})" x-on:unload.window="clearInterval(interval)">
			...
			<template x-for="(movie, index) in movies" :key="index">
				...
				<div x-show="current == index"
					...
					<!-- Movie poster -->
					<img :src="'{{URL('images/movies')}}' + '/' + movie.banner_url" alt="Movie Poster"
					     class="md:w-full h-full md:h-auto object-cover">
					<!-- End movie poster -->
					...
					<div class="absolute inset-0 bg-gray-950 bg-opacity-60 flex flex-col items-center justify-center w-full h-full">
						<div class="flex flex-col gap-2 md:gap-4 align-center justify-center text-center text-white w-80 md:w-[30rem] h-48 md:h-96 ">
							...
							<p class="text-sm md:text-xl line-clamp-3 md:line-clamp-none text-center"
							   x-text="movie.description"></p>

							<div class="flex flex-row justify-center gap-4 md:gap-6 md:mt-4 text-sm md:text-base mx-6">
								<a :href="'https://youtube.com/'" target="_blank" rel="noopener noreferrer"
								   class="bg-transparent border w-1/2 rounded-2xl border-white text-white hover:bg-gray-950 hover:text-yellow-500 px-2 py-1 md:px-4 md:py-2 backdrop-blur-sm">
									<i class="fa-brands fa-youtube"></i> <span class="hidden md:inline-flex">Vidi</span>
									Trailer
								</a>
								@clientorguest
								<a :href="routes['movies.screening.index'].replace('_ID_', movie.id)"
								   class="bg-transparent border w-1/2 rounded-2xl border-white text-white hover:bg-gray-950 hover:text-yellow-500 px-2 py-1 md:px-4 md:py-2 backdrop-blur-sm">
									<i class="fa-solid fa-ticket"></i>
									<span class="hidden md:inline-flex">Rezerviši</span> Kart<span
											class="hidden md:inline-flex">u</span><span
											class="md:hidden inline-flex">a</span>
								</a>
								@endclientorguest
			...
			<!-- Buttons -->
			<button class="absolute inset-y-0 left-5 md:left-10 px-2  h-full text-white opacity-40 hover:opacity-100 hover:cursor-pointer  z-10"
			        x-on:click="prev(), manualMove()">
				<i class="fa-solid text-lg  md:text-7xl fa-chevron-left"></i>
			</button>
			<button class="absolute inset-y-0 right-5 md:right-10 px-2  h-full text-white opacity-40 hover:opacity-100 hover:cursor-pointer  z-10"
		...
		<div class="h-96 mb-4 hidden md:flex">
			<div id="image-track" data-mouse-down-at="0" data-prev-percentage="0" class="image-paralax-wrap">
				@foreach ($genres as $genre)
					<img class="image cursor-grab w-[19rem] h-[26rem]"
					     src="{{URL('images/genres/' . $genre->image_url)}}" id="image" draggable="false"/>
				@endforeach
			</div>
		</div>
		<!-- CTA -->
		...
		<div class="hidden md:flex justify-center items-center">
			<a href="{{route('movies.index')}}"
			   class="bg-transparent border rounded-xl border-white text-white hover:bg-gray-950 hover:text-yellow-500 px-2 py-1 md:px-3 md:py-2 ">
		...
		<div class="flex flex-col md:hidden gap-6 mx-4">
			@foreach ($genres as $genre)
				<a class="overflow-hidden relative cursor-pointer" href="{{route('movies.index', ['genre' => $genre->id])}}">
		...
		<h2 class="text-4xl text-white font-bold md:mb-6">Oglašavanje <span
					class="hidden md:inline-block">i rentiranje</span></h2>
			...
			<div class="flex p-4 rounded-2xl bg-business-pattern bg-no-repeat bg-cover mx-4 md:mx-40 h-[32rem]">
				...
				<div class="md:w-1/2 flex flex-col gap-12 md:gap-4 p-2 md:p-4 items-center justify-around">
					...
					<div x-data="{ count1: 0, count2: 0, count3: 0, target: 100000, isIntersecting: false }"
						...
						<!-- END Counter -->
						<!-- Call to action -->
						<a class="border border-white text-center text-white rounded-xl px-3 py-2 font-medium w-full hover:border-red-600 hover:text-red-600"
						   href="{{route('adverts.create')}}">Započni saradnju</a>
						<!-- END Call to action -->
					</div>
					<!-- END Text wrapper -->
				</div>

<div x-data="{ showFilters: false, showSearchBar: false }">
    ...
    <form wire:submit.prevent="search" x-cloak class="text-white">
        <!-- Search Submit-->
        <button class="fixed top-20 right-6 pt-2 hover:text-red-700 z-30" type="submit" x-on:click="showFilters = false" @click="$nextTick(() => window.scrollTo(0,0))" x-show="showSearchBar">
        ...
        <!--Hidden search bar-->
        <!-- Close icon-->
        <a href="" class="fa-solid fa-arrow-right fa-2xl fixed top-24 pt-2 right-72 z-30" x-show="showSearchBar" x-on:click.prevent="showSearchBar = !showSearchBar"></a>
        <!-- End close icon -->
        <input type="text" name="searchBar" placeholder="Film, Režiser ili Žanr" x-show="showSearchBar" x-cloak x-transition-opacity wire:model="search_query" class="fixed top-20 right-4 bg-gray-950 bg-opacity-75 border-2 border-gray-200 rounded-3xl p-2 w-64 z-30">
        <!-- End search bar -->
    </form>
    <!-- End search open -->
    <!--End Search -->
    ...
    <form wire:submit.prevent="submit" class="flex flex-col gap-4 border-2 border-gray-800 bg-gray-950 bg-opacity-90 p-6 md:mr-9 md:w-[10rem] min-w-[250px] text-white" x-show="showFilters" x-transition.opacity x-ref="filters" x-on:click="showSearchBar = false">
        ...
        <div class="flex flex-col gap-2 mt-2">
            ...
            <!-- Sort criteria -->
            <select wire:model="sort_by" name="sortby" class="text-black  bg-gray-200 cursor-pointer">
                <option value="title">Nazivu</option>
                <option value="director">Režiseru</option>
                <option value="release_date">Godini</option>
            </select>
            <!-- Sort direction -->
            <select wire:model="sort_direction" name="sortdir" class="text-black bg-gray-200 cursor-pointer">

    ...
    <div class="flex flex-row flex-wrap justify-center gap-10 ">
        @forelse($movie_list as $key => $movie)
            ...
            <a href="{{route('movie.screenings.index', $movie->id)}}"
               x-data="{ showDetails: false, isMdScreen: false}"
               x-breakpoint="if($isBreakpoint('md+')) isMdScreen = true"
               x-on:mouseenter="showDetails = isMdScreen ? true : showDetails"
               x-on:mouseleave="showDetails = isMdScreen ? false : showDetails"
               x-intersect.full="showDetails = isMdScreen ? showDetails : true"
               x-intersect:leave="showDetails = isMdScreen ? showDetails : false"
               class="relative h-[28rem] md:h-[26rem]  w-[19rem] md:w-[18rem]">
                ...
                <div
                    ...
                    x-cloak
                    class="absolute bottom-0 w-full h-full bg-black bg-opacity-60 flex flex-col justify-center items-center">
                    <!-- Tech-->
                    ...
                    <!-- Details -->
                    <h2 class="text-white font-extrabold text-xl text-center">{{ $movie->title }}
                        ({{ $movie->release_year}})</h2>
                    ...
                    <div class="flex flex-col gap-2 justify-center absolute inset-x-0 text-center bottom-8 text-white ">
                        <p class="hover:text-red-500 underline">Rezervišite kartu <i
                                class="fa-solid fa-ticket underline"></i></p>

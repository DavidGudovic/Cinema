                ...
                <div class="flex flex-col gap-2 md:gap-4">
                    @for ($column = 1; $column <= $screening->hall->columns; $column++)
                        @if(in_array([$row, $column], $takenSeats))
                            <div
                                class="w-6 h-6 text-[0.7rem] pt-[0.3rem] border border-opacity-80 border-red-700 opacity-50 rounded-t-xl align-center justify-center flex">
                                <i class="fa-solid fa-xmark"></i>
                            </div>
                        @else
                            <div wire:click="toggleSeat({{ $row }}, {{ $column }})"
                                 class="w-6 h-6 text-[0.7rem] pt-[0.3rem] border border-opacity-80 border-white rounded-t-xl cursor-pointer hover:bg-white @if($this->isSelected($row, $column)) bg-green-300 @endif">
                                <span
                                    class=" w-full align-center justify-center flex select-none"> {{ chr(64 + $column) }}{{ $row }}</span>
                            </div>
                        @endif
                    @endfor
                </div>
                    ...
                    @for ($column = 1; $column <= $screening->hall->columns; $column++)
                        ...
                        @else
                            <div wire:click="toggleSeat({{ $row }}, {{ $column }})"
                                 class="w-6 h-6 text-[0.7rem] pt-[0.3rem] border border-opacity-80 border-white rounded-t-xl cursor-pointer hover:bg-white @if($this->isSelected($row, $column)) bg-green-300 @endif">
                                <span
                                    class=" w-full align-center justify-center flex "> {{ chr(64 + $column) }}{{ $row }}</span>
                            </div>
                        @endif
                    @endfor
                </div>

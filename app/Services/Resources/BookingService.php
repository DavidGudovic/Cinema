...
class BookingService implements CanExport
{
    ...
    public function massCancelOnIntersect(array $dates, array $times, int $hall_id, RequestableService $requestableService): int
    {
        ...
        $bookings = $this->filterIntersected($bookings, $times);

        foreach ($bookings as $booking) {
            $requestableService->changeRequestStatus($booking->businessRequest, Status::REJECTED, config('settings.messages.request_cancelled_intersect'));
    ...
    /**
     * Filters out bookings that intersect with the given times
     *
     * @param Collection $bookings
     * @param array $times
     * @return Collection
     */
    public function filterIntersected(Collection $bookings, array $times): Collection
    {
        return $bookings->filter(function ($booking) use ($times) {
    ...
    /**
     * Prepares a booking array|Collection for export, adds a BOM, flattens the array and adds headers
     *
